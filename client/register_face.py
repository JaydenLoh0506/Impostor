#Capture photos of face for face recognition
#Captured photos will be sent to server
...
def AdjustBrightness(image: np.ndarray, value: int) -> np.ndarray:
    """
...
def AddInstruction(image: np.ndarray, text: str, position: tuple[int, int]) -> np.ndarray:
    """
...
def CapturePhoto() -> bool:
    ...
    for i in range(4):
        photos[i] = AddInstruction(photos[i], instructions[i], positions[i])
    ...
    for i in range(4, 9):
        ...
        photos[i] = AddInstruction(photos[i], random_text, random_position)
        random_instructions.remove(random_text)

    for i, photo in enumerate(photos):
        for j, brightness in enumerate(brightness_values):
            adjusted_photo: np.ndarray = AdjustBrightness(photo, brightness)
